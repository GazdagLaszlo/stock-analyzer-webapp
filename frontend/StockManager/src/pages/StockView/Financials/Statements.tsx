...
const Statements = ({ stock }: { stock?: StockDto }) => {
  ...
  useEffect(() => {
    if (stock?.symbol) {
      api.StockReports.apiStockReportGetStockReportsToSymbolSymbolGet(
        stock?.symbol
      )
        .then((res) => {
          setReports(res.data);
  ...
  const itemLabels = (reportSection: string) => {
    ...
    const concepts2 = ['NetIncomeLoss', 'Revenues'];

    return labels.map((label, i) => ({
      label,
      concept1: concepts1[i],
      concept2: concepts2[i],
    }));
  };
  ...
  return (
    <div className="statements box">
      <p className="has-text-weight-bold my-5 subtitle is-5">
        {stock?.symbol} income statement
      </p>
      <hr />
        ...
        <table className="table is-fullwidth">
          ...
          <tbody>
            ...
            {itemLabels('ic').map(({ label, concept1, concept2 }) => (
              <tr key={label}>
                <td className="sticky-col" style={{ minWidth: '250px' }}>
                  {label}
                </td>
                {reports.map((report) => {
                  const item = report.stockReportItems?.find(
                    (x) =>
                      x.reportSection === 'ic' &&
                      (x.concept === concept1 || x.concept === concept2)
                  );
                  return (
                    <td key={report.year} style={{ minWidth: '100px' }}>
