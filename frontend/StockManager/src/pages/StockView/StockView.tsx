...
import type { StockDto, StockQuote, WatchListItemCreateDto } from "../../../generated-sources/openapi";
    ...
    */
    ...
    const addToWatchlist = async () => {
        try{
            if(!onWatchlist && stock?.id){
                api.WatchListItem.apiWatchListItemCreatePost({StockId: stock?.id} as WatchListItemCreateDto);
                ...
                api.WatchList.apiWatchListGetByUserIdGet()
                    .then(res => {
                        if(res.data.watchListItems){
                            const item = res.data.watchListItems.find(x => x.stock?.id == stock.id)
    ...
    return <>
        <div className='stockview'>
            ...
            <nav className='panel'>                          
                <div className='panel-block is-flex is-justify-content-space-between is-align-items-center has-text-weight-bold py-6 px-5' style={{height: "40vh"}}>
                    <div className='is-flex is-align-items-flex-end'>
                        <figure className='image'>
                            <img className='border-radius-10' style={{height:"25vh"}} src={`https://static2.finnhub.io/file/publicdatany/finnhubimage/stock_logo/${stock?.symbol}.png`} alt={`${stock?.symbol} logo`} />
                        </figure>
                        <div className='ml-5'>
                    ...
                    <div className='is-flex is-justify-content-space-between is-flex-direction-column' style={{height:"25vh"}}>
                        <div className='is-flex is-justify-content-end'>
                            {onWatchlist ? (
                                <button className='button is-dark' onClick={removeFromWatchlist}>
