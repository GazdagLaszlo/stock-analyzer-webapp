    ...
    const rowRenderer = useCallback(({ key, index, style }: {key: string; index: number; style: React.CSSProperties}) => {
        ...
        return (
            <div
                ...
                style={{ ...style }}
                onClick={() => navigate(`${stock.symbol}`, { state: { stockId: stock.id } })}
            >
        ...
        <div className="stocks is-flex is-flex-direction-column is-align-items-center">
            ...
            <div className="field" style={{width: "70%"}}>
                <div className="control has-icons-left">
                    <input type="text" className="input pl-6" placeholder="Keresés..." ref={inputRef}
                        ...
                        {({width, height}: {width: number; height: number}) => (
                            <List
                                ...
                                onRowsRendered={({ startIndex, stopIndex } : {startIndex: number; stopIndex: number}) => {
                                    const visible = filteredStocks.slice(startIndex, stopIndex + 1).map(s => s.symbol ?? "");
